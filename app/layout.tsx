...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return (
    <html lang="pt-BR" className={`${geistSans.variable} ${geistMono.variable} h-full`}>
      <body className="min-h-full flex" style={{ background: "#0F172A" }}>
        <aside style={{ width: 200, background: "#1E293B", minHeight: "100vh", padding: "24px 0", display: "flex", flexDirection: "column", gap: 4, borderRight: "1px solid #ffffff10", flexShrink: 0 }}>
          ...
          {navItems.map(item => (
            <Link key={item.href} href={item.href} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", color: "#94A3B8", textDecoration: "none", fontSize: 14, borderRadius: 8, margin: "0 8px" }}
