...
const COLUNAS: { id: Coluna; label: string; cor: string; bg: string }[] = [
  { id: 'todo',  label: '📋 A Fazer',     cor: '#6B7280', bg: '#F9FAFB' },
  { id: 'doing', label: '⚡ Em Andamento', cor: '#F59E0B', bg: '#FFFBEB' },
  { id: 'done',  label: '✅ Concluído',   cor: '#10B981', bg: '#ECFDF5' },
]
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
          {COLUNAS.map(col => (
            <div
              ...
              style={{ background: '#1E293B', borderRadius: 16, padding: 16, minHeight: 400,
                border: `2px solid ${col.id === 'doing' ? '#F59E0B33' : '#ffffff10'}` }}
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {cardsPorColuna(col.id).map(card => (
                  <div
                    ...
                    style={{
                      ...
                      border: '1px solid #ffffff15', cursor: 'grab',
                      borderLeft: `4px solid ${PRIORIDADE_COR[card.prioridade]}`,
                      transition: 'transform 0.1s',
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <p style={{ color: '#F1F5F9', fontWeight: 600, fontSize: 14, margin: 0, flex: 1 }}>{card.titulo}</p>
                      <button onClick={() => deletarCard(card.id)}
                        style={{ background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontSize: 16, padding: '0 0 0 8px' }}>
                        ×
                      </button>
                    </div>
                    {card.descricao && (
                      <p style={{ color: '#64748B', fontSize: 12, margin: '6px 0 0' }}>{card.descricao}</p>
                    )}
                    <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                      {card.cliente && (
                        <span style={{ background: '#1E3A8A33', color: '#93C5FD', borderRadius: 6, padding: '2px 8px', fontSize: 11 }}>
                          👤 {card.cliente}
                        </span>
                      )}
                      ...
                      {card.data_limite && (
                        <span style={{ background: '#ffffff10', color: '#94A3B8', borderRadius: 6, padding: '2px 8px', fontSize: 11 }}>
                          📅 {new Date(card.data_limite).toLocaleDateString('pt-BR')}
                        </span>
                      )}
                    </div>
                    {/* Mover rápido */}
                    <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                      {COLUNAS.filter(c => c.id !== col.id).map(c => (
                        <button key={c.id} onClick={() => moverCard(card.id, c.id)}
                          style={{ background: '#ffffff08', border: '1px solid #ffffff15', color: '#94A3B8',
                ...
                {cardsPorColuna(col.id).length === 0 && (
                  <div style={{ color: '#334155', textAlign: 'center', padding: '40px 0', fontSize: 13 }}>
                    Arraste cards aqui
                  </div>
                )}
              </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: '#000000aa', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#1E293B', borderRadius: 20, padding: 32, width: 480, border: '1px solid #ffffff15' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <input placeholder="Cliente (opcional)" value={form.cliente} onChange={e => setForm({...form, cliente: e.target.value})}
                style={inputStyle} />
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
              <button onClick={() => setModal(false)} style={{ flex: 1, background: '#0F172A', color: '#94A3B8', border: '1px solid #ffffff20', borderRadius: 10, padding: '12px', cursor: 'pointer', fontWeight: 600 }}>
                Cancelar
              </button>
              <button onClick={criarCard} style={{ flex: 2, background: 'linear-gradient(135deg, #1E3A8A, #2563EB)', color: '#fff', border: 'none', borderRadius: 10, padding: '12px', cursor: 'pointer', fontWeight: 700, fontSize: 15 }}>
                Criar Card
              </button>
            </div>
          </div>
