  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0F172A', fontFamily: "'Montserrat', sans-serif", padding: '24px' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ color: '#F59E0B', fontSize: 28, fontWeight: 800, margin: 0 }}>💾 Notas Rápidas</h1>
        <p style={{ color: '#94A3B8', fontSize: 14, margin: '4px 0 0' }}>
          Salve ideias, lembretes e informações — também via WhatsApp com <strong style={{color:'#F59E0B'}}>SALVAR: texto</strong>
        </p>
      </div>
      ...
      <div style={{ background: '#1E293B', borderRadius: 16, padding: 24, marginBottom: 32, border: '1px solid #F59E0B33' }}>
        <textarea
          ...
          rows={4}
          style={{
            width: '100%', background: '#0F172A', border: '1px solid #ffffff20',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 14 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {CATEGORIAS.map(cat => (
              <button key={cat} onClick={() => setCategoria(cat)}
                style={{
                  background: categoria === cat ? `${CAT_COR[cat]}33` : '#0F172A',
                  color: categoria === cat ? CAT_COR[cat] : '#64748B',
                  border: `1px solid ${categoria === cat ? CAT_COR[cat] : '#ffffff15'}`,
                  borderRadius: 20, padding: '4px 12px', fontSize: 12,
                  cursor: 'pointer', fontWeight: categoria === cat ? 700 : 400,
                  transition: 'all 0.2s'
                }}>
                {cat}
              </button>
          ...
          <button onClick={salvarNota} disabled={salvando || !texto.trim()}
            style={{
              background: texto.trim() ? 'linear-gradient(135deg, #1E3A8A, #2563EB)' : '#1E293B',
              color: texto.trim() ? '#fff' : '#475569',
              border: 'none', borderRadius: 10, padding: '10px 24px',
              fontWeight: 700, fontSize: 14, cursor: texto.trim() ? 'pointer' : 'not-allowed',
              transition: 'all 0.2s'
            }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        <input
          ...
          style={{
            background: '#1E293B', border: '1px solid #ffffff20', borderRadius: 10,
            padding: '8px 14px', color: '#F1F5F9', fontSize: 14, outline: 'none',
            fontFamily: "'Montserrat', sans-serif", width: 260
          }}
        />
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
      ...
      {loading ? (
        ...
        <div style={{ color: '#334155', textAlign: 'center', padding: 60, fontSize: 16 }}>
          {busca ? '🔍 Nenhuma nota encontrada' : '💾 Nenhuma nota ainda — escreva algo acima!'}
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
          {notasFiltradas.map(nota => (
            <div key={nota.id} style={{
              ...
              border: `1px solid ${CAT_COR[nota.categoria] || '#ffffff15'}22`,
              borderTop: `3px solid ${CAT_COR[nota.categoria] || '#6B7280'}`,
              position: 'relative'
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {nota.origem === 'whatsapp' && (
                    <span style={{ background: '#10B98122', color: '#10B981', borderRadius: 20, padding: '2px 8px', fontSize: 10 }}>
                      📱 WhatsApp
                    </span>
                  )}
                </div>
                <button onClick={() => deletarNota(nota.id)}
